...
impl IbdStageTracker {
    ...
    pub fn status(&self, stage: Stage) -> Result<StageStatus, StageTrackingError> {
        Ok(self.load_checkpoint()?.stage(stage).map(|progress| progress.status).unwrap_or(StageStatus::NotStarted))
    }
    ...
    pub fn mark_verified(&self, stage: Stage, completed_units: u64, total_units: Option<u64>) -> Result<(), StageTrackingError> {
        ...
        self.write_progress(StageProgress::new(stage).with_status(StageStatus::Verified).with_progress(completed_units, total_units))
    }
    ...
    pub fn mark_committed(&self, stage: Stage, completed_units: u64, total_units: Option<u64>) -> Result<(), StageTrackingError> {
        ...
        self.write_progress(StageProgress::new(stage).with_status(StageStatus::Committed).with_progress(completed_units, total_units))
    }
